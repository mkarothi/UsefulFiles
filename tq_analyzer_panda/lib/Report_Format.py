...
config='C:\\tq_analyzer_panda\config.csv'

def formatter(path):
	report_path = path + '\Report.csv'
	outpath = path + '\Infra_Analysis_Result.xlsx'
	data = pd.read_csv(report_path)
	writer = pd.ExcelWriter(outpath, engine='xlsxwriter')
	data.to_excel(writer, sheet_name='Infra_Analysis_Result', index=False)
	data1 = pd.read_csv(config)
	#writer = pd.ExcelWriter(outpath, engine='xlsxwriter')
	data1.to_excel(writer, sheet_name='Threshold_Detailed_Definition', index=False)
